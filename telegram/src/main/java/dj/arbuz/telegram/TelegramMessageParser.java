...
public class TelegramMessageParser {
    ...
    public static String parseMessageTextToHtml(String parseString) {
        ...
        Matcher urlLinkMatcher= urlLinkRegex.matcher(parseString);
        ...
        while (urlLinkMatcher.find()) {
            parsedStringBuilder.replace(0, parsedStringBuilder.length(), urlLinkMatcher.replaceFirst(createInlineLink(urlLinkMatcher.group("link"), urlLinkMatcher.group("name"))));
            urlLinkMatcher = urlLinkRegex.matcher(parsedStringBuilder);
        }
