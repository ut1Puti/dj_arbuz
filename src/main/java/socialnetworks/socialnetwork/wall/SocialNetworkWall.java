 ...
 * @version 1.0
 * @param <T> тип данных, в котором представлены посты в реализации api социальной сети
 */
public interface SocialNetworkWall<T> {
    /**
    ...
    /**
     * Метод для получения {@code amountOfPosts} постов из группы в социальной сети в виде объектов типа {@code T},
     * представляющих собой класс постов в реализации api
     *
     * @param groupScreenName имя группы в социальной сети
     * @param amountOfPosts кол-во запрашиваемых постов
     * @param userCallingMethod пользователь вызвавший метод
     * @return посты из группы в социальной сети представленные списком объектов типа {@code T},
     * которые в свою очередь представляют реализацию постов в реализации api
     * @throws SocialNetworkException возникает при ошибке обращения к api социальной сети
     * @throws SocialNetworkAuthException возникает при ошибке аутентификации пользователя
     ...
     */
    List<T> getPosts(String groupScreenName, int amountOfPosts, Actor userCallingMethod)
            throws SocialNetworkException, SocialNetworkAuthException;
}
