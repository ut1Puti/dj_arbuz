    ...
    public MessageHandlerResponse handleMessage(String message, String userSendResponseId, StoppableByUser stoppableByUserThread) {
        ...
        if (isItNoArgCommand(commandAndArgs)) {
            switch (commandAndArgs[COMMAND_INDEX]) {
                ...
                case "/auth" -> {
                    MessageTelegramHandler getAuthResponse = new GetAuthResponse(socialNetwork);
                    return getAuthResponse.sendMessage(null, userSendResponseId);
                }
                case "/stop" -> {
                    return getStopResponse(stoppableByUserThread, userSendResponseId);
                }
                case "/subscribed" -> {
                    MessageTelegramHandler getUserSubscribedGroupsLinks = new GetUserSubscribedGroupsLinks(usersBase, groupsBase);
                    return getUserSubscribedGroupsLinks.sendMessage(null, userSendResponseId);
                }
                default -> {
...
//                    return getFiveLastPosts(commandAndArgs[ARG_INDEX], userSendResponseId);
    ...
    private MessageHandlerResponse postToAllUsers(String usersSendMessage, String userReceivedMessageId) {
        return MessageHandlerResponse.newBuilder()
                                     .textMessage(usersSendMessage)
                                     .build(usersBase.getAllUsersId());
    }
    /**
